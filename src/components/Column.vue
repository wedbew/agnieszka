    ...
    classes() {
      ...
      for (let i = 0; i < breakpoints.length; i += 1) {
        ...
        if (typeof value === 'boolean') {
          // auto-column
          classnames.push(`bx--col-${name}`);
        } else if (typeof value === 'number') {
          classnames.push(`bx--col-${name}-${value}`);
        } else {
          const { span, offset } = value;
          if (typeof span === 'boolean') {
            classnames.push(`bx--col-${name}`);
          } else if (typeof span === 'number') {
            classnames.push(`bx--col-${name}-${span}`);
          }
          if (typeof offset === 'number') {
            classnames.push(`bx--offset-${name}-${offset}`);
          }
        }
      }
      if (classnames.length === 0) {
        // no breakpoints were defined. use auto-column
        classnames.push(`bx--col`);
      }
      return classnames;
    },
  },
